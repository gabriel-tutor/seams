...
## The eval suite, 2026-09-19

`claude plugin eval` (Claude Code 2.1.269 or later) ran the eight `routing` and `gate` cases from this clone on Claude Code 2.1.278 with `--allow-tools Edit Write`, on the machine above; the results are in [plugin-behavior-tests.md](plugin-behavior-tests.md). Two facts about the environment it needs:

- **The Bash sandbox and Docker Desktop.** A run granted `Bash` uses Claude Code's OS-level sandbox, which on this Mac refuses to start: `~/.docker` holds symbolic links (Docker Desktop's `cli-plugins/`), and the sandbox declines to run when it cannot reliably exclude that credential store. The two `shell`-tagged cases therefore did not run here; on a machine without those links (or Linux with `bubblewrap` and `socat`) they take `--allow-tools Bash`.
- **Where a run finds Matt Pocock's skills.** The scaffold copies them into the run's own config directory, relying on the runner's layout as observed on 2.1.278 (a throwaway `$HOME` beside a `config` directory). If a later Claude Code changes that layout, the run's session-start line reports the skills missing and the cases that route to his bare-named skills degrade; the scaffold names the missing ones on stderr.

...
1. **Every question first,** in one round in this session after every checkout (four per call): the trust questions from the Gate, and, for each pull request whose head worktree defines services (a compose file) that its end-to-end suite needs, whether to start them.
2. **Start the subagents,** four at a time (installs and test suites compete for the machine; start the next as one finishes), each with the Agent tool (general-purpose, in the background).
3. **Give each one facts only:** the pull request's facts from the Gate, its trust and services answers, `$EVID` and its two worktrees, the repository's path, the viewer's login, and a free port of its own for anything it serves. No review hints: do not summarize the diff for it or suggest what to look for; its review is its own.
4. **Its instruction:** read `<this skill's base directory>/SKILL.md` and carry out its Understand, Checks, Review and Draft sections for this one pull request, with three differences. It never asks the user anything: a decision it cannot make becomes a question in the review, or a line under not verified. It never posts, and never pushes, merges or edits anything on GitHub. It walks every review axis itself, one after another: Standards and Spec as Matt Pocock's `code-review` describes them (its SKILL.md is in the user's Claude config directory, under `skills/code-review/`), then the risk axes. It ends by writing `$EVID/review.json` and replying with the verdict in one line.
5. **A subagent that fails or times out:** write the reason to `$EVID/error.txt`. That pull request reads "not yet: could not review" and the rest go on.
